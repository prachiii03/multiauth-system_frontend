...
export default function Sidebar() {
  ...
  return (
    <div className="h-screen bg-white border-r border-gray-200 sticky top-0">
          ...
          {navigation.map((item) => (
            <Can key={item.name} permission={item.permission}>
              <button
                ...
                `}
              >
                <span className={`mr-3 ${isActive(item.href) ? 'text-blue-600' : 'text-gray-400 group-hover:text-gray-500'}`}>
                  {item.icon}
                </span>
                {item.name}
              </button>
